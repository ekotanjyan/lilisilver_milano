            ...
            $(document).ready(function(){
                ...
                function add()
                {
                    $("#availableItems option:selected").each(function(i){
                        ...
                        if (val == "PRODUCT")
                        {
                            val = prompt('{l s="Indicate the ID number for the product" mod='labmegamenus' js=1}');
                            if (val == null || val == "" || isNaN(val))
                                return;
                            text = '{l s="Product ID #" mod='labmegamenus' js=1}'+val;
                            val = "PRD"+val;
                        }
                ...
                function move(up)
                {
                    ...
                    if (tomove.length >1)
                    {
                        alert('{l s="Please select just one item" mod='labmegamenus'}');
                        return false;
                    }
                    if (up)
