...
if (!defined('_CAN_LOAD_FILES_') AND _PS_VERSION_ > '1.5')
	exit;
class labtestimonial extends ObjectModel{
	public $id_labtestimonial;
	public $name_post;
	public $email;
	public $company;
	public $address;
	...
	public static $definition = array(
        'table' => 'labtestimonial',
        'primary' => 'id_labtestimonial',
        'multilang' => false,
        'multishop' => true,
        'fields' => array(
	...
	public function add($autodate = true, $null_values = false){
		...
		$res=true ;
		if ($this->position <= 0)
			$this->position = labtestimonial::getHigherPosition() + 1;
			$res = parent::add($autodate , $null_values);
			$res &= Db::getInstance()->execute('
				INSERT INTO `'._DB_PREFIX_.'labtestimonial_shop`(`id_labtestimonial`,`id_shop`)
	...
	public function delete(){
		...
		$res &= parent::delete();
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'labtestimonial_shop`
			WHERE `id_shop` = '.(int)$this->id
		);
		if($res){
			if(file_exists(_PS_IMG_DIR_.$this->media))
			@unlink(_PS_IMG_DIR_.'labtestimonial/'.$this->media);
			return true;
		}
	}
	...
	public function deleteImage($force_delete = false) {
		...
		if ($res) {
		if(file_exists(_PS_IMG_DIR_.'labtestimonial/'.$this->media))
			@unlink(_PS_IMG_DIR_.'labtestimonial/'.$this->media);
			return true;
		}
		return $res;
	...
	public static function getAllTestimonials($p = 1, $n = false, $id = false, $excpt_id = false){
		...
		$sql= 'SELECT * FROM '._DB_PREFIX_.'labtestimonial lt ';
		$sql .=' INNER JOIN '._DB_PREFIX_.'labtestimonial_shop ls ON (lt.id_labtestimonial = ls.id_labtestimonial) ';
        $sql .=' WHERE lt.active = 1 AND ls.id_shop ='.$id_shop .( $id ? ' AND lt.id_labtestimonial ='.(int)$id :'').
        ( $excpt_id ? ' AND lt.id_labtestimonial != '.(int)$excpt_id :'');
	...
	public function updatePosition($way, $position){
		if (!$res = Db::getInstance()->executeS('
			SELECT `id_labtestimonial`, `position`
			FROM `'._DB_PREFIX_.'labtestimonial`
			ORDER BY `position` ASC'
		))
		return false;
		foreach ($res as $testimonial)
		if ((int)$testimonial['id_labtestimonial'] == (int)$this->id)
		...
		return (Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'labtestimonial`
		...
		&& Db::getInstance()->execute('
			...
			WHERE `id_labtestimonial` = '.(int)$moved_testimonial['id_labtestimonial']));
	}
	/**
	...
	public static function cleanPositions()
	{
		...
		$return = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'labtestimonial`
			SET `position` = '.(int)$i++.'
			WHERE `id_labtestimonial` = '.(int)$value['id_labtestimonial']);
		return $return;
	}
	...
	{
		$sql = 'SELECT MAX(`position`)FROM `'._DB_PREFIX_.'labtestimonial`';
