...
include_once(_PS_MODULE_DIR_.'labtestimonials/labtestimonials.php');
include_once(_PS_MODULE_DIR_.'labtestimonials/classes/LabFileUploader.php');
...
class AdminTestimonialsController extends ModuleAdminController {
	...
	protected $position_identifier = 'id_labtestimonial';
	public function __construct(){
		$this->table = 'labtestimonial';
		$this->className = 'labtestimonial';
		...
		$this->fields_list = array(
			'id_labtestimonial'=> array('title' => $this->l('ID'), 'width' => 20),
	...
	public function initToolbar(){
		if (empty($this->display)){
			...
			$this->toolbar_btn['edit'] = array(
				'href' => 'index.php?controller=AdminModules&token='.Tools::getAdminTokenLite('AdminModules') .'&configure=labtestimonials&tab_module=others&module_name=labtestimonials',
	...
	public function postProcess(){
		...
		if(isset ($_FILES['media']['name']) && ($_FILES['media']['name'])!= null ){
			$upload = new LabFileUploader($this->module, $_FILES['media']);
	...
	public function processForceDeleteImage()
	{
		...
		$update = new labtestimonial($obj->id);
		$update->media = null;
	...
	public function ajaxProcessUpdatePositions()
	{
		...
		$id_labtestimonial = (int)(Tools::getValue('id'));
		$positions = Tools::getValue($this->table);
        foreach ($positions as $position => $value){
            $pos = explode('_', $value);
            if (isset($pos[2]) && (int)$pos[2] === $id_labtestimonial){
                if ($labtestimonial = new labtestimonial((int)$pos[2])){
                    if (isset($position) && $labtestimonial->updatePosition($way, $position))
                        echo 'ok position '.(int)$position.' for carrier '.(int)$pos[1].'\r\n';
                    else
                        echo '{"hasError" : true, "errors" : "Can not update carrier '.(int)$id_labtestimonial.' to position '.(int)$position.' "}';
                }else
                    echo '{"hasError" : true, "errors" : "This carrier ('.(int)$id_labtestimonial.') can t be loaded"}';
                break;
            }
        }
	}
	...
	public function processPosition(){
        ...
        else if (!Validate::isLoadedObject($object = new labtestimonial((int)Tools::getValue($this->identifier, Tools::getValue('id_labtestimonial', 1)))))
	...
	public function renderForm($isMainTab = true) {
		...
		if (in_array($obj->media_type,$img_types)){
            $media_desc = '<br/>
            <img id="image_desc" style="clear:both;border:1px solid black;" alt="" src="'._PS_IMG_.'labtestimonial'.'/'.$obj->media.'" />
            <br/>';
