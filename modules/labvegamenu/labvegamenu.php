...
class Labvegamenu extends Module {
    private $_html = '';
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Vertical vega Menu Customer');
        $this->description = $this->l('block config');

        $this->confirmUninstall = $this->l('Are you sure you want to uninstall?');
        $this->admin_tpl_path = _PS_MODULE_DIR_ . $this->name . '/views/templates/admin/';
        if (!Configuration::get('Labvegamenu'))
    ...
    public function install() {
		...
		foreach (Language::getLanguages() as $language)
            $tab->name[$language['id_lang']] = $this->l('Vega menu');
        $tab->class_name = 'Adminlabvegamenu';
        $tab->id_parent = (int)Tab::getIdFromClassName('AdminLabMenu'); 
        $tab->module = $this->name;
        $tab->add();
		
        Configuration::updateGlobalValue('LABVEGAMENU_ITEMS', 'CAT3,CAT4,CAT5');
    ...
    public function uninstall() {
		$tab = new Tab((int)Tab::getIdFromClassName('Adminlabvegamenu'));
        $tab->delete();
		
        Configuration::updateGlobalValue('LABVEGAMENU_ITEMS', 'CAT1');
    ...
    public function getContent() {
       // $this->_html .= '<h2>' . $this->displayName . '</h2>';
        ...
        if (Tools::isSubmit('submitlabvegamenu')) {
        ...
        } else if (Tools::isSubmit('submitLabitemverMenu')) {
            $items = Tools::getValue('items');
            if (is_array($items) && count($items))
                $updated = Configuration::updateValue('LABVEGAMENU_ITEMS', (string)implode(',', $items));
            else
                $updated = Configuration::updateValue('LABVEGAMENU_ITEMS', '');
            if ($updated)
		...
		}else if (Tools::isSubmit('submitBlocktopvegaLinks')){
            ...
            if ($count_links_label || $count_label)
            {
                ...
                {
                    VegaTopLinks::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
		...
		else if (Tools::isSubmit('deletelinksmegatop'))
		{
			...
			Configuration::updateValue('LABVEGAMENU_ITEMS', str_replace(array('LNK'.$id_linksvegatop.',', 'LNK'.$id_linksvegatop), '', Configuration::get('LABVEGAMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		}
        elseif (Tools::isSubmit('updatelinksvegatop'))
        {
            ...
            if (Tools::isSubmit('updatelink'))
            {
                ...
                VegaTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_linksvegatop);
    ...
    {
        $links = VegaTopLinks::gets((int)$this->context->language->id, null, (int)Shop::getContextShopID());
        $fields_list = array(
            'id_linksvegatop' => array(
        ...
        $helper->identifier = 'id_linksvegatop';
        $helper->table = 'linksvegatop';
    ...
    public  function _displayForm() {
        ...
        if (Tools::isSubmit('submitBlocktopvegaEdit'))
        {
            $id_linksvegatop = (int)Tools::getValue('id_linksvegatop', 0);
        ...
        $helper->submit_action = 'submitlabvegamenu';
        $helper->module = $this;
    ...
    public function renderMenuForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l(' Menu Vertical Link'),
                    'icon' => 'icon-link'
                ),
                'input' => array(
                    array(
    ...
    public function renderAddLinkForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => (Tools::getIsset('updatelinksvegatop') && !Tools::getValue('updatelinksvegatop')) ? $this->l('Update link') : $this->l('Add a new link'),
                    'icon' => 'icon-link'
                ),
                'input' => array(
                    array(
        ...
        if (Tools::getIsset('updatelinksvegatop') && !Tools::getValue('updatelinksvegatop'))
            $fields_form['form']['submit'] = array(
                'name' => 'updatelinksvegatop',
                'title' => $this->l('Update')
            );

        if (Tools::isSubmit('updatelinksvegatop'))
        {
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_linksvegatop');
    ...
    public function getConfigFieldsValues()
    {
        return array(
            ...
            'show_icon' => Tools::getValue('show_icon', Configuration::get($this->name . '_show_icon')),
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updatelinksvegatop'))
        {
            $link = vegaTopLinks::getLinkLang(Tools::getValue('id_linksvegatop'), (int)Shop::getContextShopID());
        ...
        $fields_values = array(
            ...
            'id_linksvegatop' => Tools::getValue('id_linksvegatop'),
        );
...
//var_dump($cateCurrent_new);die;
    ...
    public function getStaticBlockContent($blockId = NULL, $task = NULL) {
        ...
        if (count($staticBlock) > 0) {
            ...
            $description = str_replace('/lab_milano/',__PS_BASE_URI__,$description);
            $html .= $description;
        }
        if ($task == 'item') {
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id) {
        ...
        $id = $category;
        // --- Static Block ---
        $blockId = sprintf('lab_ver_menu_idcat_%d', $id); // --- static block key
        $blockHtml = $this->getStaticBlockContent($blockId);
        /*check block right*/
        $blockIdRight = sprintf('lab_ver_menu_idcat_%d_right', $id); // --- static block key
        $blockHtmlRight = $this->getStaticBlockContent($blockIdRight);
        if($blockHtmlRight) $blockHtml = $blockHtmlRight;
        
        //$rightmenu = Mage::getStoreConfig('vvegamenu/general/rightmenu');
        ...
        if ($drawPopup)
        {  
            $html[] = '<div id="ver_menu' . $id . '" class="lab_vermenu labSub ' . $active . '" >';
        }
        else
        {
            $html[] = '<div id="ver_menu' . $id . '" class="lab_vermenu">';
        }
        // --- Top Menu Item ---
		$id_lang =  (int)Context::getContext()->language->id;
			...
			if (count($files) > 0)
			{
				...
				foreach ($files as $file){
					if (preg_match('/^'.$cate->id.'-([0-9])?_thumb.jpg/i',$file) === 1) { // set image categories 
						$html[] = '<span class="icon icon'. $_count .'"><img src="'.$this->context->link->getMediaLink(_THEME_CAT_DIR_.$file).'" alt= ""/></span>';
						$_count = $_count + 1;
					}
				}
			}
    ...
    public function drawMenuItem($children, $level = 1, $columChunk = 0, $lang_id = 1) {
        $html = '<div class="itemverMenu level' . $level . '">';
        $keyCurrent = NULL;
        if (isset($_GET['id_category'])) {
        ...
        foreach ($children as $child) {
            ...
            if (count($child) > 0) {
                ...
                if (in_array('CAT'.$child['id_category'], $arr_catsid)) {
                    $html.= '<h4 class="itemverMenuName level' . $level . $active . $ClassNoChildren . '"><span>' . $name . '</span></h4>';
                } else {
                    $html.= '<a class="itemverMenuName level' . $level . $active . $ClassNoChildren . '" href="' . $link . '"><span>' . $name . '</span></a>';
                }
    ...
    public function drawColumns($children, $id = NULL, $id_lang=NULL)
    {
        ...
        $blockHtml = '';
        $id_shop = (int) Context::getContext()->shop->id;
        $blockId = sprintf('lab_ver_menu_idcat_%d', $id);
        $blockHtml = $this->getStaticBlockContent($blockId);;
        /*Check blog right*/
        $blockIdRight = sprintf('lab_ver_menu_idcat_%d_right', $id); // --- static block key
        $blockHtmlRight = $this->getStaticBlockContent($blockIdRight);
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/lab_milano/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
            $html[] = '<div id="ver_lab_vermenu' . $id . '" class="lab_vermenu">';
        } else {
            $html[] = '<div id="ver_lab_vermenu' . $id . '" class="lab_vermenu">';
        }
        // --- Top Menu Item ---
...
//        $html[] = '</a>';
    ...
    public function hookDisplayHeader() {
        $this->context->controller->addCSS($this->_path . 'css/labvegamenu.css');
    ...
    public function drawCustomMenuMobile($blockId, $bc) {
        ...
        $blockHtml = str_replace('/labprestashop/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
            $html[] = '<li id="lab_vermenu ' . $id . '">';
        } else {
            $html[] = '<li id="lab_vermenu ' . $id . '">';
        }
        // --- Top Menu Item ---
...
//var_dump( $this->getCMSOptions(0, 1, $this->context->language->id));die;
        ...
        $links = VegaTopLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
        foreach ($links as $link)
        {
            if ($link['label'] == '')
            {
                ...
                if (!in_array('LNK'.(int)$link[0]['id_linksvegatop'], $items))
                    $html .= '<option value="LNK'.(int)$link[0]['id_linksvegatop'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
            }
            elseif (!in_array('LNK'.(int)$link['id_linksvegatop'], $items))
                $html .= '<option value="LNK'.(int)$link['id_linksvegatop'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
        }
        $html .= '</optgroup>';
        $html .= '</select>';
        return $html;
...
//var_dump(substr($item, 0, strlen($value[1])));
			...
			{	case 'CAT':
                ...
                case 'PRD':
                    ...
                    if (!is_null($product->id))
                        $this->_menuLink .= '<div id ="lab_vermenu_product" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($product->getLink()).'"><span>'.$product->name.'</span></a></div></div>'.PHP_EOL;
                    $this->_menuLinkMobile .= '<li id ="lab_vermenu_product" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($product->getLink()).'"><span>'.$product->name.'</span></a></div></li>'.PHP_EOL;
                    break;
                case 'NEWPRODUCT':
                    ...
                    $this->_menuLink .= '<div id ="lab_newproducts" class="lab_vermenu labmenuproducts"><div class="parentMenu" ><a href="'.__PS_BASE_URI__."new-products".'"><span>' . $this->l('New Product') . ' </span><span class="icon-new">' . $this->l('New') . '</span></a>';
                    ...
                    foreach ($newProducts as $key=>$newProduct){
                        ...
                        $this->_menuLink .= '<div class ="column item-product col'. $count .'"><div class="item" ><div class="img_produc"><a title="'.$newProduct['name'].'" class="product-img" href="'.$newProduct['link'].'"> <img src="'.$imagePath.'" alt="'.$newProduct['name'].'" /></a>
                           <p class="price">'. $Currency .' '. Tools::ps_round($newProduct['price'],2) .'</p>';
                        if($newProduct['reduction']!=0){
                            $this->_menuLink .= '<p class="old-price"> '.  $Currency.Tools::ps_round($newProduct['price_without_reduction'], 2).'</p>'.PHP_EOL;
                        }
                        $this->_menuLink .= ' </div>
                    ...
                    foreach ($newProducts as $newProduct){
                        ...
                        $this->_menuLinkMobile .= '<li><div class="img_produc"><a title="'.$newProduct['name'].'" class="product-img" href="'.$newProduct['link'].'"> <img src="'.$imagePath.'" alt="'.$newProduct['name'].'" /></a>
                           <p class="price">'. $Currency .' '.Tools::ps_round($newProduct['price'],2)  .'</p>';
                        if($newProduct['reduction']!=0){
                            $this->_menuLinkMobile .= '<p class="old-price"> '.  $Currency.Tools::ps_round($newProduct['price_without_reduction'], 2).'</p>'.PHP_EOL;
                        }
                        $this->_menuLinkMobile .= '</div>
						   <div class="right"><h2 class="produc-name"><a title="'.$newProduct['name'].'" href="'.$newProduct['link'].'"><span>'.$newProduct['name'].'</span></a></h2>
						    <a class="lnk_more" href="'.$newProduct['link'].'" title="' . $this->l('View') . '">' . $this->l('View') . '</a></div>
						   </li>'.PHP_EOL;
                    }
                ...
                case 'SALEPRODUCT':
                    ...
                    $this->_menuLink .= '<div id ="lab_saleProducts" class="lab_vermenu labmenuproducts"><div class="parentMenu" ><a href="'.__PS_BASE_URI__."prices-drop".'"><span>' . $this->l(' Sale') . '</span><span class="icon-hot">' . $this->l('Hot') . '</span></a>';
                    $this->_menuLink .= '</div>'.PHP_EOL;
                    $this->_menuLink .= '<div class ="popup" style ="display:none">';
                    $this->_menuLink .= '<div class ="block1">';
                    ...
                    foreach ($SaleProducts as $key => $SaleProduct){
                        ...
                        $this->_menuLink .= '<div class ="column item-product col'. $count .'"><div class="item" ><div class="img_produc"><a title="'.$SaleProduct['name'].'" class="product-img" href="'.$SaleProduct['link'].'"> <img src="'.$imagePath.'" alt=""/></a>
							        <p class="price"> '. $Currency. Tools::ps_round($SaleProduct['price'],2) .'</p>'.PHP_EOL;
                        if($SaleProduct['reduction']!=0){
                            $this->_menuLink .= '<p class="old-price"> '.  $Currency.Tools::ps_round($SaleProduct['price_without_reduction'], 2).'</p>'.PHP_EOL;
                        }
                        $this->_menuLink .='</div>
                    ...
                    foreach ($SaleProducts as $SaleProduct){
                        ...
                        $imagePath = Link::getImageLink($SaleProduct['link_rewrite'], $SaleProduct['id_image'], 'small_default');
                        $this->_menuLinkMobile .= '<div class="list_products"><div class="img_produc"><a title="'.$SaleProduct['name'].'" class="product-img" href="'.$SaleProduct['link'].'"> <img  src="'.$imagePath.'" alt="'.$SaleProduct['name'].'"/></a>
							<p class="price">'. $Currency .' '.Tools::ps_round($SaleProduct['price'],2) .'</p>'.PHP_EOL;
                        if($SaleProduct['reduction']!=0){
                            $this->_menuLinkMobile .= '<p class="old-price"> '.  $Currency.Tools::ps_round($SaleProduct['price_without_reduction'], 2).'</p>'.PHP_EOL;
                        }
                        $this->_menuLinkMobile .= '	</div>
                ...
                case 'CMS':
                    ...
                    if (count($cms))
                        $this->_menuLink .= '<div id ="lab_vermenu_cms" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($cms[0]['link']).'"><span>'.$cms[0]['meta_title'].'</span></a></div></div>'.PHP_EOL;
                ...
                case 'CMS_CAT':
                    ...
                    if (Validate::isLoadedObject($category))
                        $this->_menuLink .= '<div id ="lab_vermenu_cms" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'"><span>'.$category->name.'</span></a>';
                    $this->_menuLink .= '</div>'.PHP_EOL;
                    $this->_menuLink .= $this->getCMSMenuItems($category->id);

                    $this->_menuLinkMobile .= '<li class="lab_vermenu_cms"><a href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'"><span>'.$category->name.'</span></a>';
                ...
                case 'ALLMAN':
                    ...
                    $link = new Link;
                    $this->_menuLink .= '<div id ="lab_vermenu_allsub" class="lab_vermenu"><div class="parentMenu" ><a href="'.$link->getPageLink('manufacturer').'" ><span>'.$this->l('Manufacturers').'</span></a></div>'.PHP_EOL;
                    ...
                    $this->_menuLinkMobile .= '<li id ="lab_vermenu_allsub"><a href="'.$link->getPageLink('manufacturer').'" ><span>'.$this->l('Manufacturers').'</span></a>'.PHP_EOL;
                    $this->_menuLinkMobile .= '<ul class ="popup">';
                    foreach ($manufacturers as $key => $manufacturer)
                ...
                case 'MAN':
                    ...
                    if (!is_null($manufacturer->id))
                    {
                        ...
                        $link = new Link;
                        $this->_menuLink .= '<div id ="lab_vermenu_man" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'"><span>'.$manufacturer->name.'</span></a></div></div>'.PHP_EOL;
                        $this->_menuLinkMobile .= '<li id ="lab_vermenu_man" class="lab_vermenu"><a href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'"><span>'.$manufacturer->name.'</span></a></li>'.PHP_EOL;
                ...
                case 'ALLSUP':
                    $link = new Link;
                    $this->_menuLink .= '<div id ="lab_vermenu_allsub" class="lab_vermenu"><div class="parentMenu" ><a href="'.$link->getPageLink('supplier').'" ><span>'.$this->l('All suppliers').'</span></a></div>'.PHP_EOL;
                    ...
                    $this->_menuLinkMobile .= '<li id ="lab_vermenu_allsub"><a href="'.$link->getPageLink('supplier').'" ><span>'.$this->l('All suppliers').'</span></a>'.PHP_EOL;
                    $this->_menuLinkMobile .= '<ul class ="popup">';
                    foreach ($suppliers as $key => $supplier)
                ...
                case 'SUP':
                    ...
                    if (!is_null($supplier->id))
                    {
                        ...
                        $this->_menuLink .= '<div id ="lab_vermenu_sub" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'"><span>'.$supplier->name.'</span></a></div></div>'.PHP_EOL;
                        $this->_menuLinkMobile .= '<li id ="lab_vermenu_sub" class="lab_vermenu"><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'"><span>'.$supplier->name.'</span></a></li>'.PHP_EOL;
                ...
                case 'SHOP':
                    ...
                    {
                        $link = new Link;
                        $this->_menuLink .= '<div id ="lab_vermenu_sub" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'"><span>'.$supplier->name.'</span></a></div></div>'.PHP_EOL;
                        $this->_menuLinkMobile .= '<li id ="lab_vermenu_sub" class="lab_vermenu"><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'"><span>'.$supplier->name.'</span></a></li>'.PHP_EOL;
                    }
                    break;
                case 'LNK':
                    ...
                    if (count($link))
                    {
                        ...
                        $this->_menuLink .= '<div id ="lab_vermenu_link" class="lab_vermenu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</a></span></div></div>'.PHP_EOL;
                        $this->_menuLinkMobile .= '<li id ="lab_vermenu_link" class="lab_vermenu"><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</a></span></li>'.PHP_EOL;
                    }
                    break;
    ...
    public function drawCustomMenuItemMobile($category, $level = 0, $last = false, $item, $lang_id) {
        ...
        $id_shop = (int) Context::getContext()->shop->id;
        $id = $category;
        $blockId = sprintf('lab_vermenu_idcat_%d', $id);
        $staticBlock = $this->getStaticBlockContent($blockId);
        $blockIdRight = sprintf('lab_vermenu_idcat_%d_right', $id);
        ...
        $drawPopup = ($blockHtml || count($activeChildren));
        if ($drawPopup) {
            $htmlmobile[] = '<li id="lab_vermenu' . $id . '" class="lab_vermenu' . $active . ' nav-' . $item . '">';
      ...
      //  $homeCates = $category->getHomeCategories($lang_id);
        ...
        if ($showhome) {
            ...
            if ($page_name == 'index')
                $active = ' act';
            $id = "_home";
            $html .= '<div id="ver_lab_vermenu' . $id . '" class="lab_vermenu' . $active . '">';
            $html .= '<div class="parentMenu">';
   ...
   /*      foreach ($homeCates as $cate) {
        ...
        return $this->display(__FILE__, 'labvegamenu.tpl');
    }
    ...
    private function getMenuItems()
    {
        ...
        {
            $conf = Configuration::get('LABVEGAMENU_ITEMS');
//print_r(Configuration::get('labcategoryslider_list_cate'));die;
            if (strlen($conf))
                return explode(',', Configuration::get('LABVEGAMENU_ITEMS'));
            else
                return array();
        }
    }
