<?php
class labtabcategoryslider extends Module {
	private $spacer_size = '5';	
	private $_postErrors  = array();	
	public function __construct() {
		$this->name 		= 'labtabcategoryslider';
		$this->tab 			= 'front_office_features';
		$this->version 		= '1.6';
        $this->bootstrap    = true;
        $this->_html        = '';
		$this->author 		= 'labersthemes';
		$this->displayName 	= $this->l('Lab Tab Category Products Slider');
		$this->description 	= $this->l('Lab Tab Category Products Slider');
		parent :: __construct();
	}
	...
	Configuration::updateGlobalValue($this->name.'_list_cate',$cateDefault);
		return parent :: install()
			...
			&& $this->registerHook('manufacturer');
	}

      public function uninstall() {
		Configuration::deleteByName($this->name . '_list_cate');
        $this->_clearCache('labtabcategoryslider.tpl');
    ...
    public function hookHeader($params){
        if ($this->psversion()==5){
            $this->context->controller->addCSS(($this->_path).'labtabcategoryslider.css', 'all');
        } else {
            Tools::addCSS(($this->_path).'labtabcategoryslider.css');
        }
    }
    // Hook Home
	public function hookmanufacturer($params) {
		...
		return $this->display(__FILE__, 'labtabcategoryslider.tpl');
	}
