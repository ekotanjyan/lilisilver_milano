{capture name=path}<a href="{smartblog::GetSmartBlogLink('smartblog')}">{l s='Blog' mod='smartblog'}</a>
    {if $title_category != ''}
		<span class="navigation-pipe">{$navigationPipe}</span>{$title_category}
	{/if}
{/capture}
	{if $title_category != ''}
		<h1 class="title_category"><span>{$title_category}</span></h1>
	{else}
		<h1 class="title_category"><span>{l s='Blog' mod='smartblog'}</span></h1>
	{/if}
    {if $postcategory == ''}
        {if $title_category != ''}
