...
{if count($currencies) > 1}
	<div id="currencies-block-top" class="pull-left dropdown">
		<form id="setCurrency" action="{$request_uri}" method="post">
			<div class="current">
				...
				{foreach from=$currencies key=k item=f_currency}
					{if $cookie->id_currency == $f_currency.id_currency}<span>{$f_currency.iso_code}</span><i class="icon icon-angle-down"></i>{/if}
				{/foreach}
			</div>
