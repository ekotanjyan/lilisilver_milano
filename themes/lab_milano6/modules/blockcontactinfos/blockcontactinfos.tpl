	...
	<div>
        ...
        <ul class="toggle-footer">
            {if $blockcontactinfos_company != ''}
            	<li>
            		<i class="pe-7s-map-marker"></i> <!-- {$blockcontactinfos_company|escape:'html':'UTF-8'}{if $blockcontactinfos_address != ''}: --> <span>{$blockcontactinfos_address|escape:'html':'UTF-8'}</span>{/if}
            	</li>
            {/if}
            {if $blockcontactinfos_phone != ''}
            	<li>
            		<i class="pe-7s-call"></i><!-- {l s='Call:' mod='blockcontactinfos'}  -->
            		<span>{$blockcontactinfos_phone|escape:'html':'UTF-8'}</span>
            	</li>
            {/if}
            {if $blockcontactinfos_email != ''}
            	<li>
            		<i class="pe-7s-mail"></i><!-- {l s='Email:' mod='blockcontactinfos'}  -->
