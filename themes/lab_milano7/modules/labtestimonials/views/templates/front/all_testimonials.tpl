...
{capture name=path}
    <span class="navigation-pipe">{$navigationPipe}</span>{l s='Testimonial' mod='labtestimonials'}
{/capture}
<div class="content-testimonials block">
    <h4 class="title_block">{l s='Testimonial' mod='labtestimonials'}</h4>
    <a class="button btn btn-default button-small" href="{$link->getModuleLink('labtestimonials','views',['process' => 'form_submit'])}"><span>{l s='Submit Testimonial' mod='labtestimonials'}</span></a>
    {if $id != 0}
        <h3>{l s='Current testimonial' mod='labtestimonials'}</h3>
    {/if}
    ...
    {if isset($page_other_testimonials) && $page_other_testimonials}
        <h3>{l s='Other testimonials' mod='labtestimonials'}</h3>
    ...
    <div id="pagination" class="pagination">
            ...
            <ul class="pagination">
                {if $page != 1}
                    ...
                    <li id="pagination_previous">
                        <a href="{testimonialpaginationlink p=$p_previous n=$nbpagination}" title="{l s='Previous' mod='testimoninals'}" rel="nofollow">&laquo;&nbsp;{l s='Previous' mod='labtestimonials'}</a>
                    </li>
                {else}
                    <li id="pagination_previous" class="disabled"><span>&laquo;&nbsp;{l s='Previous' mod='labtestimonials'}</span></li>
                {/if}
                {if $page > 2}
                ...
                {if $alltestimoninals|@count > $page * $nbpagination}
                    ...
                    <li id="pagination_next"><a href="{testimonialpaginationlink p=$p_next n=$nbpagination}" title="Next" rel="nofollow">{l s='Next' mod='labtestimonials'}&nbsp;&raquo;</a></li>
                {else}
                    <li id="pagination_next" class="disabled"><span>{l s='Next' mod='labtestimonials'}&nbsp;&raquo;</span></li>
                {/if}
            </ul>
        {/if}
        {if $alltestimoninals|@count > 10}
            <form action="{$pagination_link}" method="get" class="pagination">
                <p>
                    <input type="submit" class="button_mini" value="{l s='OK' mod='labtestimonials'}" />
