...
{if $page_name =='index'}
	...
	<div class="lab-nivoSlideshow">
		...
		<div id="htmlcaption{$slide.id_slide}" class=" nivo-html-caption nivo-caption">
			...
			<div class="title">
				{if $slide.url}<a href="{$slide.url}" title="{$slide.title}">{/if}{$slide.title|escape:'html':'UTF-8'}{if $slide.url}</a>{/if}
			</div>
			{/if}
			{if $slide.legend}
			<div class="lab_caption">
				<h4>{$slide.legend|escape:'html':'UTF-8'}</h4>
			</div>
			{/if}
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$(".labbannerSlide .lab_caption h4").lettering();
		$(".labbannerSlide .title a").lettering();
		$(".description h5").lettering();
	});
    </script>
